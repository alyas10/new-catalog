...
-  [`**System.Windows**`](http://System.Windows)`**.Forms.Control**` -- для визуальных элементов.

-  `**System.ComponentModel.Component**` -- для невизуальных элементов. Экземпляры компонентов -- это объекты данных классов, существующие в виде полей класса формы (`Form`). **Компонент = Состояние (свойства) + Действия (методы) + Обратные связи (события).**
